...
def fredf_loss(pred: torch.Tensor, target: torch.Tensor, fourier_weight: float) -> torch.Tensor:
    ...
    # 
    # implementation per the fredf paper (wang and his friends 2025)
    ...
    assert pred.shape == target.shape, "Prediction and target shapes must be the same."
    assert pred.dim() == 3, "Prediction and target must be 3D tensors of shape (batches, covariates, forecast_horizon). Don't ever let me catch you making this mistake again."
